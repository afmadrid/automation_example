...
def test_001_panic_cancelled_by_driver(pre_conditions):
    ...
    # 3. Verify event code 205 was generated (ex: Athena check)
    Report.step('Event code verified.')
    Report.verify(True, True, 'Event code 205 was not generated.')
    # 4. Hold down panic button until LED turns off
    Report.step('Panic button pressed for 2 seconds.')
    Report.verify(True, True, 'Panic LED is turned on.')
    # 5. Check the state of the pan/priv accumulator
    Report.step('Status of pan/priv accumulator verified.')
    Report.verify(True, True, 'Pan/priv accumulator is disabled.')
    # 6. Verify event code 207 was generated (ex: Athena check)
    Report.step('Event code verified.')
    Report.verify(True, True, 'Event code 207 was not generated.')
    sleep(randint(0, 5))
